...
from swift.common.common.utils import reiterate, split_path, Timestamp, pairs
from swift.common.common.exceptions import InvalidTimestamp
...
class Response(object):
    ...
    def __init__(self, body=None, status=200, headers=None, app_iter=None,
                 request=None, conditional_response=False,range=None,
    ...
    def _response_iter(self, app_iter, body):
        if self.conditional_response and self.request and \
                self.range and self.range.ranges and \
                not self.content_range:
            ranges = self.range.ranges_for_length(self.content_length)
            if ranges == []:
                self.status = 416
